...
fun minAndMax(number: Int): Int {
    val numberArray = number.toString().map { it.toString().toInt() }
    var minimum = numberArray.sorted().take(1)
    var maximum = numberArray.sortedDescending().take(1)
    var sum = minimum.sum() + maximum.sum()
    return sum
}
